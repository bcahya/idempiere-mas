...
public class MLCOInvoiceWithholding extends X_LCO_InvoiceWithholding
{
	 ...
	 * @return
	 */
	public static MLCOInvoiceWithholding[] getOf(Properties ctx, int C_Invoice_ID, String whereClause, String trxName)
	{
		MLCOInvoiceWithholding[] invWth = null;
		
		if(null == whereClause)
			whereClause = "C_Invoice_ID=?";
		else
			whereClause = "C_Invoice_ID=? " + whereClause;

		List<MLCOInvoiceWithholding> invWthList = 
				new Query(ctx, MLCOInvoiceWithholding.Table_Name, whereClause, trxName)
				.setParameters(C_Invoice_ID)
				.setOnlyActiveRecords(true)
				.list();
		
		invWth = new MLCOInvoiceWithholding[invWthList.size()];
		invWthList.toArray(invWth);
