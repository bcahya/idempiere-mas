...
public class LCO_MInvoice extends MInvoice
{
	...
	public int recalcWithholdings() {
		...
		try
		{
			...
			// OrgInfo variables
			MOrgInfo oi = MOrgInfo.get(getCtx(), getAD_Org_ID(), false, get_TrxName());
			...
			while (rst.next())
			{
				...
				StringBuffer sqlr = new StringBuffer("SELECT LCO_WithholdingRule_ID "
						...
						+ " WHERE (IsDefault='Y' OR IsForNPWPPartner=" + (bp.isNPWPPartner()? "'Y')" : "'N')")
...
//					if (!wc.isCalcOnInvoice()) {
//						log.severe("Withholding calculated not on invoice "+rsr.getInt(1));
//						continue;
//					}
					...
					if (wc.getBaseType() == null) {
						...
						if (wrc.isUseWithholdingCategory() && wrc.isUseProductTaxCategory()) {
							...
							// base = all lines
							sqllca = 
								"SELECT SUM (LineNetAmt), il.C_InvoiceLine_ID "
								+ "  FROM C_InvoiceLine il "
								+ " WHERE IsActive='Y' AND il.C_Invoice_ID = ? "
								+ " GROUP BY il.C_InvoiceLine_ID";
						}
						...
						while (rslca.next()){
							...
							log.info("Base: "+base+ " Invoice Line: "+c_invoiceline_id+ " Thresholdmin:"+wc.getThresholdmin()); 
									...
									tax.getRate().compareTo(Env.ZERO) != 0) 
							{
								...
								BigDecimal taxamt = createWithholdingLines(base, c_invoiceline_id, wc, tax, wr, wt);
										
								totwith = totwith.add(taxamt);
								noins++;
							}
							
							// Reset base to null, so that it will not be repeated again
							base = null;
							// ECS_LCO
						}
						rslca.close();
			...
			MTax tax, X_LCO_WithholdingRule wr, X_LCO_WithholdingType wt)
	{
		...
		iwh.setAddToGrandTotal(wt.isAddToGrandTotal());
