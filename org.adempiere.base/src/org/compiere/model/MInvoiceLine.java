...
public class MInvoiceLine extends X_C_InvoiceLine
{
	...
	private boolean m_has_called = false;
	 ...
	 * 	@param C_BPartner_ID business partner
	 */
	public void setPrice (int M_PriceList_ID, int C_BPartner_ID, int C_BPartner_Location_ID)
	{
		if (getM_Product_ID() == 0 || isDescription())
			return;
		//
		if (log.isLoggable(Level.FINE)) log.fine("M_PriceList_ID=" + M_PriceList_ID);
		m_productPricing = (IProductPricing) new MProductPricing (getM_Product_ID(),
			C_BPartner_ID, getQtyInvoiced(), m_IsSOTrx, C_BPartner_Location_ID);
		m_productPricing.setM_PriceList_ID(M_PriceList_ID);
	...
	{
		if (!m_reversal && !m_has_called)
		{
			String sql = "SELECT Reversal_ID FROM C_Invoice WHERE C_Invoice_ID = ?";
			int retVal = DB.getSQLValue(get_TrxName(), sql, getC_Invoice_ID());
			if (retVal > 0)
			{
				m_reversal = true;
			}
	...
	private boolean m_enforcePriceLimit = true;

	public void setIsEnforcePriceLimit(boolean enforcePriceLimit) {
		m_enforcePriceLimit = enforcePriceLimit;
	}

	private boolean isEnforcePriceLimit() {
