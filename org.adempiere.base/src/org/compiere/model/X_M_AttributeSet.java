...
public class X_M_AttributeSet extends PO implements I_M_AttributeSet, I_Persistent 
{
	 ...
	 * Set Is For UPC.
	 * 
	 * @param IsUPC Is it for UPC attribute recording? If checked on Attribute Set
	 *              the UPC is mandatory on every product transaction.
	 */
	public void setIsUPC(boolean IsUPC) {
		set_Value(COLUMNNAME_IsUPC, Boolean.valueOf(IsUPC));
	}

	/**
	 * Get Is For UPC.
	 * 
	 * @return Is it for UPC attribute recording? If checked on Attribute Set the
	 *         UPC is mandatory on every product transaction.
	 */
	public boolean isUPC() {
		Object oo = get_Value(COLUMNNAME_IsUPC);
		if (oo != null) {
			if (oo instanceof Boolean)
				return ((Boolean) oo).booleanValue();
			return "Y".equals(oo);
	...
	public org.compiere.model.I_M_Attribute getUPCAttribute() throws RuntimeException
    {
		return (org.compiere.model.I_M_Attribute)MTable.get(getCtx(), org.compiere.model.I_M_Attribute.Table_Name)
			.getPO(getUPCAttribute_ID(), get_TrxName());	}

	/** Set UPC Attribute.
		@param UPCAttribute_ID 
		The attribute id of UPC
	  */
	public void setUPCAttribute_ID (int UPCAttribute_ID)
	{
		if (UPCAttribute_ID < 1) 
			set_Value (COLUMNNAME_UPCAttribute_ID, null);
		else 
			set_Value (COLUMNNAME_UPCAttribute_ID, Integer.valueOf(UPCAttribute_ID));
	}

	/** Get UPC Attribute.
		@return The attribute id of UPC
	  */
	public int getUPCAttribute_ID () 
	{
		Integer ii = (Integer)get_Value(COLUMNNAME_UPCAttribute_ID);
		if (ii == null)
			 return 0;
		return ii.intValue();
	}
	...
	/**
	 * Set UPC Unique Per Unit.
	 * 
	 * @param IsUniqueUPCPerUnit To indicate that the UPC is unique for every unit
	 *                           item.
	 */
	public void setIsUniqueUPCPerUnit(boolean IsUniqueUPCPerUnit) {
		set_Value(COLUMNNAME_IsUniqueUPCPerUnit, Boolean.valueOf(IsUniqueUPCPerUnit));
	}

	/**
	 * Get UPC Unique Per Unit.
	 * 
	 * @return To indicate that the UPC is unique for every unit item.
	 */
	public boolean isUniqueUPCPerUnit() {
		Object oo = get_Value(COLUMNNAME_IsUniqueUPCPerUnit);
		if (oo != null) {
			if (oo instanceof Boolean)
				return ((Boolean) oo).booleanValue();
			return "Y".equals(oo);
