...
public class MInvoiceTax extends X_C_InvoiceTax
{
	...
	//TAMBAHAN
	public static MInvoiceTax[] getByInvoice(Properties ctx, int C_Invoice_ID, String trxName)
	{
		String whereClause =COLUMNNAME_C_Invoice_ID + "=?";
		
		List<MInvoiceTax> list =
				new Query(ctx, Table_Name, whereClause, trxName).setParameters(C_Invoice_ID)
					.list();
					
		return list.toArray(new MInvoiceTax[list.size()]);
