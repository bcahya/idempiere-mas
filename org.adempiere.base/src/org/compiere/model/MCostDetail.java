...
public class MCostDetail extends X_M_CostDetail
{
	 ...
	 * @return
	 */
	public static boolean createMovementConfirm (MAcctSchema as, int AD_Org_ID, 
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int M_MovementLineConfirm_ID, int M_CostElement_ID, 
			BigDecimal Amt, BigDecimal Qty, boolean from,
			String Description, String trxName)
		{
			StringBuilder msget = new StringBuilder( "M_MovementLineConfirm_ID=? AND IsSOTrx=") 
					.append((from ? "'Y'" : "'N'")).append(" AND Coalesce(M_CostElement_ID,0)=").append(M_CostElement_ID);
			MCostDetail cd = get (as.getCtx(),msget.toString(), 
					M_MovementLineConfirm_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
			//
			if (cd == null)		//	createNew
			{
				cd = new MCostDetail (as, AD_Org_ID, 
					...
					M_CostElement_ID, 
					Amt, Qty, Description, trxName);
				cd.setM_MovementLineConfirm_ID (M_MovementLineConfirm_ID);
				cd.setIsSOTrx(from);
			}
			else
			{
