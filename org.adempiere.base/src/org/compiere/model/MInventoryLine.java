...
public class MInventoryLine extends X_M_InventoryLine 
{
	...
	//TAMBAHAN
	/**
	 * Validate quantity book if there are an unexpected movement to this locator while doing physical inventory.
	 * @return
	 */
	public String validateQtyBook(Timestamp created) 
	{
		StringBuilder sql = 
				new StringBuilder("SELECT count(*) FROM M_Transaction ")
				.append(" WHERE M_Locator_ID = ? AND M_Product_ID = ?")
				.append(" AND Created > ?");
			
		int count = DB.getSQLValue(get_TrxName(), sql.toString(), 
				getM_Locator_ID(), getM_Product_ID(), created);
		
		String msg = null;
		if (count > 0) {
			MLocator loc = MLocator.get(getCtx(), getM_Locator_ID());
			MProduct product = MProduct.get(getCtx(), getM_Product_ID());
			msg = "(" + loc.getValue() + " - " + product.getName() + ")";
		}
		return msg;
	} //validateQtyBook
	
	/**
	 * Validate quantity count based on the latest changes made by others to the product's current quantity on hand
	 * and to its current quantity on intransit customer locator. 
	 * @return
	 */
	public BigDecimal validateQtyCount()
	{
		BigDecimal prevQtyBook = getQtyBook();

		BigDecimal qtyBook = MStorageOnHand.getQtyOnHandForLocator(
				getM_Product_ID(), getM_Locator_ID(), getM_AttributeSetInstance_ID(), get_TrxName());
		
		BigDecimal qtyBookDiff = qtyBook.subtract(prevQtyBook);
		
		BigDecimal qtyCount = getQtyCount();
		
		int locIntransitCustomer_ID = 
				MWarehouse.get(getCtx(), 
							 MLocator.get(getCtx(), getM_Locator_ID()).getM_Warehouse_ID())
							 .getIntransitCustomerLocator_ID(true);
		
		BigDecimal qtyIntransitCustomer = MStorageOnHand.getQtyOnHandForLocator(
				getM_Product_ID(), locIntransitCustomer_ID, getM_AttributeSetInstance_ID(), get_TrxName());
		
		setQtyBook(qtyBook);
		setQtyIntransitCustomer(qtyIntransitCustomer);
		setTotalQtyBook(qtyBook.add(qtyIntransitCustomer));
		
		qtyCount = getQtyCount().add(qtyBookDiff);
