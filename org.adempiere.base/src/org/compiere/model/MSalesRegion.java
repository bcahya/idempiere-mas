...
public class MSalesRegion extends X_C_SalesRegion
{
	...
	//TAMBAHAN
	public static MSalesRegion getOfOrg(Properties ctx, int AD_Org_ID, String trxName)
	{
		MSalesRegion sr = null;
		String whereClause = "C_SalesRegion_ID = (SELECT C_SalesRegion_ID FROM AD_Org WHERE AD_Org_ID = ?)";
		sr = new Query(ctx, Table_Name, whereClause, trxName).setParameters(AD_Org_ID).first();
