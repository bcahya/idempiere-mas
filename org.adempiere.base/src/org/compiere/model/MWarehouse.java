...
public class MWarehouse extends X_M_Warehouse
{
	...
	//TAMBAHAN
	public static final String INITIAL_INTRANSIT_CUSTOMER_LOC = "Intransit-Customer";
	 ...
	 * @return
	 */
	public int getIntransitCustomerLocator_ID(boolean createIfNotExists)
	{
		String locValue = getValue().concat("-").concat(INITIAL_INTRANSIT_CUSTOMER_LOC);
		int loc_ID = DB.getSQLValueEx(
				get_TrxName(), 
				"SELECT M_Locator_ID FROM M_Locator WHERE M_Warehouse_ID=? AND Value=?", 
				get_ID(), locValue);
		
		if (loc_ID <= 0 && createIfNotExists)
		{
			MLocator intransitLoc = new MLocator(this, locValue);
			intransitLoc.setX(getValue());
			intransitLoc.setY(getName());
			intransitLoc.setZ(MWarehouse.INITIAL_INTRANSIT_CUSTOMER_LOC);
			intransitLoc.setIsInTransit(true);
			intransitLoc.saveEx();
			
			loc_ID = intransitLoc.get_ID();
		}
