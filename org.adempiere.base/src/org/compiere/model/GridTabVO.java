...
public class GridTabVO implements Evaluatee, Serializable
{
	...
	//	CLogger.get().warning("ShowTrl=" + showTrl + ", showAcct=" + showAcct);
		try
		{
			...
			if (vo.TableName.equals(MUNSBankAccountAccess.BANKSTATEMENT_TABLENAME))
				vo.isBankStatementTab = true;

			vo.SingleRowHeight = rs.getInt("SingleRowHeight");
			if (rs.wasNull())
				vo.SingleRowHeight = 0;

			vo.TabMRHeight = rs.getInt("TabMRHeight");
			if (rs.wasNull())
				vo.TabMRHeight = 0;
			
			vo.m_IsRecordOrderTab = rs.getString("IsRecordOrderTab").equals("Y") ? true : false;
			//if (userDef != null)
			//	vo.SingleRowHeight = userDef.getSingleRowHeight();
			
			vo.MultiRowHeight = rs.getInt("MultiRowHeight");
			if (rs.wasNull())
				vo.MultiRowHeight = 0;
			//if (userDef != null && userDef.getMul)
			//	vo.MultiRowHeight = userDef.getMultiRowHeight();			
			...
			//UNS Customization : to restrict access to only allowed records to view by current login user.
			initExtendedRecordAccess(vo, rs);
	...
	private static boolean createFields (GridTabVO mTabVO)
	{
		...
		String sql = GridFieldVO.getSQL(mTabVO.ctx, mTabVO.isFieldSortByGrid());
	...
	/* === Start UniCore Customization === */
	public int			SingleRowHeight = 0;
	public int			TabMRHeight = 0;
	public int			MultiRowHeight = 0;
	public boolean 		m_IsRecordOrderTab = false;
	public boolean 		isBankStatementTab = false;
	/* === Start UniCore Customization === */
	...
	private boolean isFieldSortByGrid = false;
	...
	protected GridTabVO clone(Properties Ctx, int windowNo)
	{
		...
		//UNS Customization
		clone.m_IsRecordOrderTab = m_IsRecordOrderTab;
		//end
	 ...
	 * @throws SQLException
	 */
	private static void initExtendedRecordAccess(GridTabVO vo, ResultSet rs) throws SQLException
	{
		StringBuilder BSWhere = new StringBuilder();
		
		if (vo.isBankStatementTab)
		{
			int AD_User_ID = Env.getContextAsInt(vo.ctx, "#AD_User_ID");
			
			if (!MUNSBankAccountAccess.isUserAgent(AD_User_ID)) {
				BSWhere
				.append(" C_BankAccount_ID IN (SELECT baa.C_BankAccount_ID FROM UNS_BankAccount_Access baa WHERE baa.AD_User_ID=")
				.append(AD_User_ID).append(")");
			}
		}
		
		if (!Util.isEmpty(vo.WhereClause, true) && BSWhere.length() > 0)
			BSWhere = new StringBuilder(" AND ").append(BSWhere);
		
		boolean hasTableAccess = rs.getString("HasTableAccess").equalsIgnoreCase("Y");
		
		if (hasTableAccess)
		{
			int currUserLogin = Env.getContextAsInt(vo.ctx, "#AD_User_ID");
			
			String sql = "SELECT WhereClause FROM UNS_Table_Access WHERE AD_User_ID=? AND AD_Table_ID=? AND IsActive='Y'";
			
			String whereClause = DB.getSQLValueString(null, sql, currUserLogin, vo.AD_Table_ID);
			
			
			if (whereClause != null && !whereClause.trim().isEmpty()) {
				if (BSWhere.length() > 0)
					BSWhere.append(" AND ");
				BSWhere.append(" (").append(whereClause).append(")");
			}
			else {
				boolean restrictedAccess = rs.getString("RestrictAccess").equalsIgnoreCase("Y");
				
				if (restrictedAccess) {
					if (BSWhere.length() > 0)
						BSWhere.append(" AND ");
					BSWhere.append(" 1 = 2"); // Don't show any record to the current logged in user.
				}
			}
		}
		if (!Util.isEmpty(BSWhere.toString())) {
			if (!Util.isEmpty(vo.WhereClause))
				vo.WhereClause += " AND ";
			vo.WhereClause += BSWhere.toString();
		}
	} // initExtendedRecordAccess
	
	/**
	 * To set if this vo's field are sorted by grid displayed flag and sequence no.
	 * @param isSortByGrid
	 */
	public void setIsFieldSortByGrid(boolean isSortByGrid)
	{
		isFieldSortByGrid = isSortByGrid;
	}
	
	/**
	 * Check if this vo's fields are sorted by grid displayed fland and sequence no.
	 * @return
	 */
	public boolean isFieldSortByGrid()
	{
		return isFieldSortByGrid;
	}
