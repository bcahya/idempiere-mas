...
// S
	 ...
	 * 
	 * @param DisbursementDate Date when payment can be processed (Cek/Giro)
	 */
...
//	throw new IllegalArgumentException ("WithholdingAmt is virtual column");
		System.out.println("Trying to set virtual column");
	}
	 ...
	 * 
	 * @param VendorBeginningBalance Vendor/Customer Balance prior to current
	 *                               payment transactions. Negative values means
	 *                               company still owed to the vendor.
	 */
	public void setVendorBeginningBalance(BigDecimal VendorBeginningBalance) {
		set_ValueNoCheck(COLUMNNAME_VendorBeginningBalance, VendorBeginningBalance);
	}

	/**
	 * Get Beginning Balance.
	 * 
	 * @return Vendor/Customer Balance prior to current payment transactions.
	 *         Negative values means company still owed to the vendor.
	 */
	public BigDecimal getVendorBeginningBalance() {
		BigDecimal bd = (BigDecimal) get_Value(COLUMNNAME_VendorBeginningBalance);
		if (bd == null)
			return Env.ZERO;
		return bd;
	}
	 ...
	 * Set Ending Balance.
	 * 
	 * @param VendorEndingBalance Vendor/Customer Balance after completing payment
	 *                            transactions. Negative values means company still
	 *                            owed to the vendor.
	 */
	public void setVendorEndingBalance(BigDecimal VendorEndingBalance) {
		set_ValueNoCheck(COLUMNNAME_VendorEndingBalance, VendorEndingBalance);
	}

	/**
	 * Get Ending Balance.
	 * 
	 * @return Vendor/Customer Balance after completing payment transactions.
	 *         Negative values means company still owed to the vendor.
	 */
	public BigDecimal getVendorEndingBalance() {
		BigDecimal bd = (BigDecimal) get_Value(COLUMNNAME_VendorEndingBalance);
		if (bd == null)
			return Env.ZERO;
		return bd;
	}
	...
	public void setUNS_PR_Allocation_ID(int UNS_PR_Allocation_ID) {
		if (UNS_PR_Allocation_ID < 1)
			set_Value(COLUMNNAME_UNS_PR_Allocation_ID, null);
		else
			set_Value(COLUMNNAME_UNS_PR_Allocation_ID, Integer.valueOf(UNS_PR_Allocation_ID));
	}
