...
public class MProductBOM extends X_M_Product_BOM
{
	...
	//TAMBAHAN
	/**
	 * Get the BOMQty used based on formula type.
	 * 
	 * @return -1 if it is OnFieldFormula, FormulaQty if it PreciseTotalOutputFormula, or BOMQty if it is 
	 * BasicFormula.
	 */
	public BigDecimal getBOMQtyBasedOnFormulaType()
	{
		BigDecimal bomQty = BigDecimal.valueOf(-1);
		
		if (getFormulaType().equals(FORMULATYPE_PreciseTotalOutputFormula))
			bomQty = getFormulaQty();
		else if (getFormulaType().equals(FORMULATYPE_BasicFormula))
			bomQty = getBOMQty();
	...
	/**
	 * -- @UntaSoft Customization ---
	 * Get BOM lines of a product for the given BOMType. Null BOMType means get all BOM Lines.
	 * 
	 * @param ctx
	 * @param M_Product_ID
	 * @param BOMType
	 ...
	 * @return
	 */
	public static MProductBOM[] getBOMLines (Properties ctx, int M_Product_ID, String BOMType, String trxName)
	{
 		//FR: [ 2214883 ] Remove SQL code and Replace for Query - red1
		String whereClause = "M_Product_ID=?";
		if (BOMType != null)
			whereClause += " AND BOMType='" + BOMType + "'";
