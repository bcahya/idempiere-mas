...
// 1
	...
	//TAMBAHAN
	/**
	 * Set Max Write Off Sales.
	 * 
	 * @param MaxWriteOffSales Max Write Off Sales
	 */
	public void setMaxWriteOffSales(BigDecimal MaxWriteOffSales) {
		set_Value(COLUMNNAME_MaxWriteOffSales, MaxWriteOffSales);
	}

	/**
	 * Get Max Write Off Sales.
	 * 
	 * @return Max Write Off Sales
	 */
	public BigDecimal getMaxWriteOffSales() {
		BigDecimal bd = (BigDecimal) get_Value(COLUMNNAME_MaxWriteOffSales);
		if (bd == null)
			return Env.ZERO;
		return bd;
	}

	public org.compiere.model.I_C_Tax getOrgTax() throws RuntimeException
    {
		return (org.compiere.model.I_C_Tax)MTable.get(getCtx(), org.compiere.model.I_C_Tax.Table_Name)
			.getPO(getOrgTax_ID(), get_TrxName());	}

	/** Set Default Org Tax.
		@param OrgTax_ID 
		The default tax to be used when user does not define the tax for the organization's transaction.
	  */
	public void setOrgTax_ID (int OrgTax_ID)
	{
		if (OrgTax_ID < 1) 
			set_Value (COLUMNNAME_OrgTax_ID, null);
		else 
			set_Value (COLUMNNAME_OrgTax_ID, Integer.valueOf(OrgTax_ID));
	}

	/** Get Default Org Tax.
		@return The default tax to be used when user does not define the tax for the organization's transaction.
	  */
	public int getOrgTax_ID () 
	{
		Integer ii = (Integer)get_Value(COLUMNNAME_OrgTax_ID);
		if (ii == null)
			 return 0;
		return ii.intValue();
	}
