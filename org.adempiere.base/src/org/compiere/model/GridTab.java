...
import com.unicore.model.I_UOMConvertion;
import com.unicore.ui.UNSMsgboxInfo;
import com.unicore.model.MUNSBankAccountAccess;
...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	/** UNS Specific for specified extension handler. */
	private String				m_extensionHandler;
	...
	/**=== Start UniCore Customization ===**/
	private boolean 			m_hasNestedTab = false;
	private UNSMsgboxInfo		m_msgboxInfo;

	/**=== End UniCore Customization ===**/
	...
	private boolean loadFields()
	{
		...
		for (int f = 0; f < m_vo.getFields().size(); f++)
		{
			...
			if (voF != null)
			{
				...
				if(!m_hasNestedTab && field.getIncluded_Tab_ID() > 0)
					m_hasNestedTab = true;
	...
	public void query (boolean onlyCurrentRows, int onlyCurrentDays, int maxRows)
	{
		...
		if (isDetail())
		{
			...
			//----- @UntaSoft Customization : ignore link column checking for included tab -----// 
			if (lc.equals("") && !isIncluded()) {
	...
	public boolean dataSaveNoFire(boolean manualCmd)
	{
		if (log.isLoggable(Level.FINE)) log.fine("#" + m_vo.TabNo + " - row=" + m_currentRow);
		try
		{
	...
	public void fireEventAfterDataSave(boolean manualCmd)
	{
		if (log.isLoggable(Level.FINE)) log.fine("#" + m_vo.TabNo + " - row=" + m_currentRow);
		try
		{
	...
	*/
	...
	public boolean isReadOnly()
	{
		...
		if (m_vo.isBankStatementTab)
		{
			Object BAObj_ID = getValue(MBankStatement.COLUMNNAME_C_BankAccount_ID);
			
			if (BAObj_ID != null) {
				Boolean baAccess = 
						MUNSBankAccountAccess.hasBankAccountAccess(
								m_vo.ctx, (int) BAObj_ID, Env.getContextAsInt(m_vo.ctx, "#AD_User_ID"));
				
				if (baAccess != null && !baAccess)
					return true;
			}
		}
	...
	/**
	 * Get the height for this tab in multi row mode.
	 * @return
	 */
	public int getTabMRHeight()
	{
		return m_vo.TabMRHeight;
	}

	/**
	 * Get the single row height for this tab.
	 * @return
	 */
	public int getSingleRowHeight()
	{
		return m_vo.SingleRowHeight;
	}

	/**
	 * Get the multi row height for this tab.
	 * @return
	 */
	public int getMultiRowHeight()
	{
		return m_vo.MultiRowHeight;
	}
	...
	//UNS Customization
	public boolean isRecordOrderTab()
	{
		return m_vo.m_IsRecordOrderTab;
	}
...
//					value = mField.getDefault();
	...
	/**
	 * Get the field that is set to include a tab.
	 * 
	 * @param AD_Tab_ID
	 * @return
	 */
	public GridField getIncludedFieldOf(int AD_Tab_ID)
	{
		GridField retField = null;
		for (GridField field : m_mTable.getFields())
		{
			if (field.getIncluded_Tab_ID() == AD_Tab_ID)
			{
				retField = field;
				break;
			}
	...
	/**
	 * To indicate if this tab has other tab(s) nested on it.
	 * 
	 * @return
	 */
	public boolean hasNestedTab()
	{
		return m_hasNestedTab;
	}
	
	/**
	 * To set if this tab's field are sorted by grid displayed flag and sequence no.
	 * @param isSortByGrid
	 */
	public void setIsFieldSortByGrid(boolean isSortByGrid)
	{
		m_vo.setIsFieldSortByGrid(isSortByGrid);
	}
	
	private GridField[] m_tabFields = null;
	
	/**
	 * Get fields for the non grid (tab) mode view.
	 * The non displayed field will be put in the end of the list.
	 * @return
	 */
	public GridField[] getTabFields()
	{
		if (m_tabFields != null)
			return m_tabFields;

		ArrayList<GridField> tabFieldList = new ArrayList<GridField>();
		
		for (GridField field : m_mTable.getFields())
		{
			tabFieldList.add(field);
		}
		
		Collections.sort(tabFieldList, new Comparator<GridField>() {

			@Override
			public int compare(GridField o1, GridField o2) 
			{
				//Pretend the non displayed field to be greater to the displayed one, in order to put it in the end of
				//the list.
		...
		m_tabFields = new GridField[tabFieldList.size()];
		tabFieldList.toArray(m_tabFields);
	 ...
	 * @return
	 */
	public GridField getTabField(int ix)
	{
		return (ix >=0)? getTabFields()[ix] : null;
	}
	
	public void setExtensionHandler(String ExtensionHandler) {
		m_extensionHandler = ExtensionHandler;
		m_mTable.setExtensionHandler(ExtensionHandler);
	}
	 ...
	 * @return
	 */
	public String customCalloutAction(GridField mField)
	{
		Object value = mField.getValue();
		Object oldValue = mField.getOldValue();
		String className = "com.unicore.util.UNSCustomCalcConvertion";
		try {
			Class<?> clazz = Class.forName(className);
			I_UOMConvertion customCalcConvertion = (I_UOMConvertion) clazz.newInstance();
			customCalcConvertion.start(m_vo.ctx, m_vo.WindowNo, this, mField, value, oldValue);
	 ...
	 * @param msgboxInfo
	 */
	public void setMsgboxInfo(UNSMsgboxInfo msgboxInfo)
	{
		this.m_msgboxInfo = msgboxInfo;
	}
	 ...
	 * @return
	 */
	public UNSMsgboxInfo getMsgboxInfo()
	{
		return m_msgboxInfo;
	} // getMsgboxInfo
