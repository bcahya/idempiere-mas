...
public class MBankStatement extends X_C_BankStatement implements DocAction
{
	...
	/**
	 * Get open Bank Statement, Create new if not exists when the value of createNew
	 * is true
	 * 
	 ...
	 * @return
	 */
	public static MBankStatement getOpen(int C_BankAccount_ID, String trxName, boolean createNew) {
		String sql = " SELECT * FROM C_BankStatement WHERE C_BankAccount_ID = "
				+ " ? AND DocStatus IN ('DR','IV') ORDER BY " + " StatementDate DESC ";

		MBankStatement bStmt = null;
		PreparedStatement st = null;
		ResultSet rs = null;

		try {
			st = DB.prepareStatement(sql, trxName);
			st.setInt(1, C_BankAccount_ID);
			rs = st.executeQuery();
			if (rs.next()) {
				bStmt = new MBankStatement(Env.getCtx(), rs, trxName);
			} else {
				sql = " SELECT MAX(StatementDate) FROM C_BankStatement WHERE "
						+ " C_BankAccount_ID = ? AND DocStatus IN ('CO','CL') ";
				Timestamp statementDate = DB.getSQLValueTS(trxName, sql, C_BankAccount_ID);
				if (null == statementDate) {
					statementDate = Env.getContextAsDate(Env.getCtx(), "#Date");
					if (null == statementDate) {
						statementDate = new Timestamp(System.currentTimeMillis());
					}
				}

				MBankAccount bAccount = new MBankAccount(Env.getCtx(), C_BankAccount_ID, trxName);
				bStmt = new MBankStatement(bAccount);
				bStmt.setDateAcct(statementDate);
				bStmt.setStatementDate(statementDate);

				String name = bAccount.getName().concat(" - ").concat(statementDate.toString());
				bStmt.setName(name);
				bStmt.setDescription("Auto Generated :: ".concat(name));
				bStmt.saveEx();
			}
	...
	public boolean deleteLines (String criteria)
	{
		if (Util.isEmpty(criteria, true))
		{
			return false;
		}
		
		StringBuilder sb = new StringBuilder("SELECT SUM(")
		.append(MBankStatementLine.COLUMNNAME_StmtAmt).append(") FROM ")
		.append(MBankStatementLine.Table_Name).append(" WHERE ")
		.append(Table_Name).append("_ID = ? AND ").append(criteria);
		
		String sql = sb.toString();
		BigDecimal sumDel = DB.getSQLValueBD(get_TrxName(), sql, get_ID());
		
		sb = new StringBuilder("UPDATE ").append(Table_Name).append(" SET ")
		.append(COLUMNNAME_EndingBalance).append(" = (")
		.append(COLUMNNAME_EndingBalance).append("- ?), ")
		.append(COLUMNNAME_StatementDifference).append(" = (")
		.append(COLUMNNAME_StatementDifference).append(" - ?) WHERE ")
		.append(COLUMNNAME_C_BankStatement_ID).append(" = ? ");
		
		sql = sb.toString();
		int updated = DB.executeUpdateEx(sql, new Object[] {sumDel,sumDel, get_ID()}, 
				get_TrxName());
		if (updated == -1)
			return false;
		
		log.log(Level.INFO, updated + " updated Bank Statement [" 
				+ getDocumentNo() + "]");
		...
		sql = sb.toString();
		int deleted = DB.executeUpdateEx(sql, new Object[] {get_ID()}, 
				get_TrxName());
		if (updated == -1)
			return false;
		
		log.log(Level.INFO, deleted + " record Statement Line deleted");
		
		String thisStatus = getDocStatus();
		if ((DOCSTATUS_Completed.equals(thisStatus)
				|| DOCSTATUS_Closed.equals(thisStatus)) && isPosted())
		{
			sb = new StringBuilder("UPDATE C_BankAccount SET ")
			.append(MBankAccount.COLUMNNAME_CurrentBalance).append(" = (")
			.append(MBankAccount.COLUMNNAME_CurrentBalance).append("- ?)")
			.append(" WHERE ").append(MBankAccount.COLUMNNAME_C_BankAccount_ID)
			.append(" = ?");
			
			sql = sb.toString();
			updated = DB.executeUpdateEx(sql, new Object[] {sumDel, 
					getC_BankAccount_ID()}, get_TrxName());
			if (updated == -1)
				return false;
			
			log.log(Level.INFO, updated + " Bank Account updated");
			String error = DocManager.postDocument(
					MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID(), 
							get_TrxName()), Table_ID, get_ID(), 
