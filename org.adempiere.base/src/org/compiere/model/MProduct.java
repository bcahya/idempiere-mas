...
public class MProduct extends X_M_Product
{
	...
	//TAMBAHAN
	/**
	 * Get a product id from product table that is matching UPCSubStr search clause.
	 * 
	 * @param ctx
	 * @param UPCSubStr
	 * @param likeClause
	 * @param trxName
	 * @return the product id or -1 if not exist.
	 */
	public static int getIDByUPC(Properties ctx, String UPCSubStr, boolean likeClause, String trxName)
	{
		String whereClause = "";
		if (likeClause)
			whereClause = "UPC LIKE '%" + UPCSubStr + "%'";
		else 
			whereClause = "UPC='" + UPCSubStr + "'";
		
		String sql = "SELECT M_Product_ID FROM M_Product WHERE " + whereClause;
		
		int M_Product_ID = DB.getSQLValueEx(trxName, sql);
