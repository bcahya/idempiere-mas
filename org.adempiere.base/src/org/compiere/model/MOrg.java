...
public class MOrg extends X_AD_Org
{
	 ...
	 * 	Get Org from Cache
	 *	@param ctx context
	 *	@param OrgValue value of org
	 *  @param trxName trxName
	 *	@return MOrg
	 */
	public static MOrg get (Properties ctx, String OrgValue, String trxName)
	{
		int AD_Org_ID = DB.getSQLValue(trxName, 
				"SELECT AD_Org_ID FROM AD_Org WHERE value LIKE '"+OrgValue+"'");
		if (AD_Org_ID==-1)
			throw new AdempiereException("Not found: organization.");
		MOrg retValue = s_cache.get (AD_Org_ID);
		if (retValue != null)
			return retValue;
		retValue = new MOrg (ctx, AD_Org_ID, null);
	...
	public MOrg getParent()
	{
		if(getInfo() == null)
			return null;
		else if(getInfo().getParent_Org_ID() <= 0)
			return null;
		
		return new MOrg(getCtx(),getInfo().getParent_Org_ID(), get_TrxName());
	}
}	//	MOrg
