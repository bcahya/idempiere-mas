...
public class MTax extends X_C_Tax
{
	...
	//TAMBAHAN
	/**
	 * Check if the tax is calculated based on document base amount.
	 * 
	 * @param iol
	 * @return
	 */
	public boolean isDocumentLevel(MOrderLine ol)
	{
		return isDocumentLevel(ol.getParent(), MOrder.COLUMNNAME_DiscountAmt);
	}
	
	/**
	 * Check if the document has discount amount greater than zero.
	 * 
	 * @param po
	 * @param AddDiscountAmtColumnName
	 * @return
	 */
	public boolean isDocumentLevel(PO po, String AddDiscountAmtColumnName)
	{
		boolean isDocLevel = super.isDocumentLevel();
		
		if (po == null || isDocLevel)
			return isDocLevel;
		
		isDocLevel = hasDocumentLevelDiscount(po, AddDiscountAmtColumnName);
	 ...
	 * @return
	 */
	public boolean hasDocumentLevelDiscount(PO po, String AddDiscountAmtColumnName)
	{
		Object docLevelDisc = po.get_Value(AddDiscountAmtColumnName);
		
		if (docLevelDisc == null)
			return false;
		
		return ((BigDecimal) docLevelDisc).signum() > 0;
	}
}	//	MTax
