 ...
 * 
 */
public class Doc_UNSQAMLabResult extends Doc {

	private MUNSQAMLabResult m_labResult = null;
	 ...
	 * @param trxName
	 */
	public Doc_UNSQAMLabResult(MAcctSchema as, Class<?> clazz, ResultSet rs,
	 ...
	 *            trx
	 */
	public Doc_UNSQAMLabResult(MAcctSchema as, ResultSet rs, String trxName) {
		super(as, MUNSQAMLabResult.class, rs, DOCTYPE_UNSQualityAssurance, trxName);
	...
	private void LoadLines() {
		...
		for(MUNSQAMLabResultLine line : m_labResult.getLines())
		{
			...
			//check product BOM
//			MUNSQALabTest labTest = MUNSQALabTest.get(line.getCtx(), line.getUNS_QALabTest_ID());
//			MProduct service = MProduct.get(line.getCtx(), labTest.getProductService_ID());
//			MProductBOM[] productBOM = MProductBOM.getBOMLines(service);
//			
//			if (productBOM.length>0)
//			{
//				for (MProductBOM bom : productBOM)
//				{
//					docLine = new DocLine(bom, this);
//					docLine.setAmount(BigDecimal.ZERO); //Default is Zero
//					list.add(docLine);
//				}
//			}
	...
	public ArrayList<Fact> createFacts(MAcctSchema as) {
		...
		if (getDocumentType().equals(I_C_DocType.DOCBASETYPE_QAD))
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (AD_OrgTrx_ID==0){
					result = new MUNSQAMLabResult(getCtx(), line.getUNS_QAMLab_Result_ID(), getTrxName());
				...
				//Check is QAService or Material				
				ProductCost qaServPC = null;
//				if (line.getUNS_QAMLab_Result_ID()>0)
//				{
...
//				else 
//				{
//					MProductBOM material = new  MProductBOM(getCtx(), line.getM_Product_BOM_ID(), getTrxName());
//					qaServPC = new ProductCost(getCtx(), material.getM_ProductBOM_ID(), 0, getTrxName());
//					qaServPC.setQty(material.getBOMQty());
//					qty = material.getBOMQty();
//				}
